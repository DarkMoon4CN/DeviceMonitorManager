//------------------------------------------------------------------------------
// <auto-generated>
//     此代码已从模板生成。
//
//     手动更改此文件可能导致应用程序出现意外的行为。
//     如果重新生成代码，将覆盖对此文件的手动更改。
// </auto-generated>
//------------------------------------------------------------------------------

namespace MonitorManager.Models
{
    using System;
    using System.Collections.Generic;
    
    public partial class YY_DATA_Statistic
    {
        public string STCD { get; set; }
        public string ItemID { get; set; }
        public string INTV { get; set; }
        public System.DateTime TM { get; set; }
        public Nullable<decimal> MaxVal { get; set; }
        public Nullable<decimal> MinVal { get; set; }
        public Nullable<decimal> AvgVal { get; set; }
        public Nullable<decimal> MSEVal { get; set; }
        public Nullable<decimal> SDVal { get; set; }
    }
}
